...
using namespace std;
BackTrackFilter::BackTrackFilter(){
	distance_threshold = 0.01;
	nr_iter = 10;
}
BackTrackFilter::~BackTrackFilter(){}

const bool debugg_BackTrackFilter = true;

Transformation * BackTrackFilter::filterTransformation(Transformation * input){
	IplImage* img_combine;
	int width;
	int height;
	if(debugg_BackTrackFilter)
	{	
		...
		cvNamedWindow("Backtrack combined image", CV_WINDOW_AUTOSIZE );
		cvShowImage("Backtrack combined image", img_combine);
		cvWaitKey(0);
...
void BackTrackFilter::print(){printf("%s\n",name.c_str());}
void BackTrackFilter::setVisualization(boost::shared_ptr<pcl::visualization::PCLVisualizer> view){viewer = view;}
