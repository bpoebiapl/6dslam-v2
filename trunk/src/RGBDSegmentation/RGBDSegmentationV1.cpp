...
RGBDSegmentationV1::RGBDSegmentationV1(Calibration * calib){
	calibration = calib;
	width 	= 640;
	height 	= 480;
...
RGBDSegmentationV1::~RGBDSegmentationV1(){
	for(int i = 0; i < width; i++)
	{
		delete[] r[i];
		delete[] g[i];
...
void RGBDSegmentationV1::disp_segments()
{
...
void RGBDSegmentationV1::disp_edge(string s , float** edge, int width,int height)
{
	IplImage * img 			= cvCreateImage(cvSize(width, height), IPL_DEPTH_32F, 3);
	float * data 			= (float *)(img->imageData);
...
void RGBDSegmentationV1::disp_xy_edge(string s , float** xedge,float** yedge, int width,int height)
{
	IplImage * img 			= cvCreateImage(cvSize(width, height), IPL_DEPTH_32F, 3);
	float * data 			= (float *)(img->imageData);
...
void RGBDSegmentationV1::directional_smoothing(float** xedge, float** yedge, float** edge, int width, int height){
...
float** RGBDSegmentationV1::getEdges()
{
...
void RGBDSegmentationV1::init(IplImage * rgb_img,IplImage * depth_img)
{    
...
void RGBDSegmentationV1::extractSurfaces(vector<segmentation_fit * > * fit_segments, vector<int> * segment_w, vector<int> * segment_h, float ** edge, int & segment_id ,int iter, int width,int height, float min)
{
...
segmentation_fit * RGBDSegmentationV1::fit_surface(vector<float> * seg_x, vector<float> * seg_y, vector<float> * seg_z)
{
	float x_sum = 0;
	float y_sum = 0;
	float z_sum = 0;
...
void RGBDSegmentationV1::extractSurfaces(vector<segmentation_fit * > * fit_segments, float ** edge, int width,int height)
{
	int segment_id = -1;
	for(int i = 0; i < width; i++){
		for(int j = 0; j < height; j++){
...
vector<segmentation_fit * > * RGBDSegmentationV1::segment(IplImage * rgb_img,IplImage * depth_img)
{
