...
RansacPCLCopySlowMatcher::RansacPCLCopySlowMatcher()
{
	name = "RansacPCLCopySlowMatcher";
	printf("new RansacPCLCopySlowMatcher\n");
	nr_iter 			= 500;
	distance_threshold 	= 0.005f;
	feature_threshold 	= 0.2f;
}

RansacPCLCopySlowMatcher::~RansacPCLCopySlowMatcher(){}

void RansacPCLCopySlowMatcher::update(){}

const bool debugg_RansacPCLCopySlowMatcher = false;
Transformation * RansacPCLCopySlowMatcher::getTransformation(RGBDFrame * src, RGBDFrame * dst)
{
	if(debugg_RansacPCLCopySlowMatcher){printf("RansacPCLCopySlowMatcher::getTransformation(%i,%i)\n",src->id,dst->id);}
	...
	for(int i = 0; i < nr_iter; i++)
	{
		...
		int counter = 0;
		while(possibleMatches[src_a]->size() == 0 && counter < 5000)										{counter++;src_a = rand() % src_nr_points;}
		while((possibleMatches[src_b]->size() == 0 || src_a == src_b)  && counter < 5000)					{counter++;src_b = rand() % src_nr_points;}
		while((possibleMatches[src_c]->size() == 0 || src_c == src_a || src_c == src_b) && counter < 5000)	{counter++;src_c = rand() % src_nr_points;}
