...
KeyPointSet::~KeyPointSet(){
	for(unsigned int i = 0; i < valid_key_points.size(); i++)
	{
		delete valid_key_points.at(i);
	}
	for(unsigned int i = 0; i < invalid_key_points.size(); i++)
	{
		delete invalid_key_points.at(i);
	}
};
