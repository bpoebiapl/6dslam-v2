...
using namespace std;
//const bool debugg_AICKv2 = false;
...
AICKv2::~AICKv2(){printf("delete AICKv2\n");}

float AICKv2::getAlpha(int iteration){return 1-initial_alpha*pow(shrinking,float(iteration));}

Transformation * AICKv2::getTransformation(RGBDFrame * src, RGBDFrame * dst)
{
	//printf("AICKv2::getTransformation(%i,%i)\n",src->id,dst->id);
	if(debugg_AICKv2){printf("AICKv2::getTransformation(%i,%i)\n",src->id,dst->id);}
	...
	//if(debugg_AICKv2){printf("src_keypoints.size() = %i, dst_keypoints.size() = %i\n",int(src_keypoints.size()),int(dst_keypoints.size()));}
...
//printf("line:%i\n",__LINE__);
	...
	Eigen::Matrix4f transformationMat = initial_guess;
	...
	for(int iter = 0; iter < nr_iter; iter++)
	{
		...
		if(debugg_AICKv2 ){
			//if(iter == nr_iter-1)
			{
				cvShowImage("combined image iteration", img_combine_clone);
				cvWaitKey(0);
			}
	...
	for(int i = 0; i < 3; i++)
	{
		for(int j = 0; j < 3; j++)
		{
			float d = transformation->transformationMatrix(i,j);
			if(i == j){d-=1;}
			diff += d*d;
		}
	}
