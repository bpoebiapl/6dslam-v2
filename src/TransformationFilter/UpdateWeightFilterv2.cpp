...
using namespace std;
UpdateWeightFilterv2::UpdateWeightFilterv2(){
	limit = 0.015f;
}
UpdateWeightFilterv2::~UpdateWeightFilterv2(){}
Transformation * UpdateWeightFilterv2::filterTransformation(Transformation * input){
	...
	IplImage* depth_img = cvLoadImage(dst->input->depth_path.c_str(),CV_LOAD_IMAGE_UNCHANGED);
	...
	//transformation->weight += input->weight;
	//transformation->weight*=0.5f;
	
	//transformation->weight *= input->weight;
	
	//printf("update weightv2 results: %f %f %f-> %f\n",counter_good,counter_bad,counter_total,transformation->weight);
	//printf("UpdateWeightFilterv2 cost: %f\n",time);
	
	return transformation;
}
void UpdateWeightFilterv2::print(){printf("%s\n",name.c_str());}
void UpdateWeightFilterv2::setVisualization(boost::shared_ptr<pcl::visualization::PCLVisualizer> view){viewer = view;}
