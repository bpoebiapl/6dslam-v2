...
void test(vector< frame_data * > * all_frames, string dataset, vector<FrameMatcher * > matchers, vector<int> backing, vector<float> individual_threshold)
{
	...
	for(int tr = 0; tr < individual_threshold.size(); tr++){
		float tres = individual_threshold.at(tr);
		for(int matcher = 0; matcher < results->size(); matcher++)
		{
			printf("%s_threshold%f_%i_%s_mat_pos = [ ",dataset.c_str(),tres,matcher,matchers.at(matcher)->name.c_str());
			...
			for(int step = 1; step < results->at(matcher)->size(); step++)
			{
				...
				for(float thresh_counter = 1; thresh_counter <= thresh_steps; thresh_counter++)
				{
					...
					for(int nr = 0; nr < current->size(); nr++)
					{
						...
						if(current->at(nr)->transformation->weight > tres){
							nr_accepted++;
							if(data->pos_error < thresh){nr_good++;}
							else						{nr_bad++;}
						}
						//printf("%f %f weight %i -> %f\n",nr_total,nr_accepted,nr,current->at(nr)->transformation->weight);
					}
				
					//printf("\n total:%f accept%f \n",nr_total,nr_accepted);
...
int main(int argc, char **argv)
{
	...
	//OrbExtractor * surf = new OrbExtractor();
	...
	map->extractor = surf;
	frames = getFrameData("/home/johane/test_data/rgbd_dataset_freiburg1_room",map,1000000);
	printf("done getting frame data\n");
	struct timeval test_start, test_end;
	done_tasks = 0;
	gettimeofday(&test_start, NULL);
	...
	matchers.push_back(new AICK(10000,25,0.8,scaling));backing.push_back(steps);
	vector<float> tr;
	for(int i = 0; i <= 700; i += 50){tr.push_back(i);}
	test(frames,"aick_surf_weighted",matchers,backing,tr);
	matchers.clear();
	backing.clear();
/*
	...
	test(frames,"originalAICKsurf",matchers,backing,40);
	matchers.clear();
	backing.clear();
...
/*	
	...
	matchers.push_back(new AICK(200,25,0.8,scaling));backing.push_back(steps);
	matchers.push_back(new AICK(200,7 ,0.5,scaling));backing.push_back(steps);
	matchers.push_back(new AICK(200,5 ,0.3,scaling));backing.push_back(steps);
	matchers.push_back(new AICK(200,3 ,0.3,scaling));backing.push_back(steps);
	matchers.push_back(new AICK(200,1 ,0.3,scaling));backing.push_back(steps);

	 
	test(frames,"originalAICKsurfIterations",matchers,backing);

	char buff[250];
	bow_path = "bow_output/library_1000_1_10_%i.feature.surf";
	words.clear();
	for(int i = 0; i < 1000; i++){sprintf(buff,bow_path.c_str(),i);words.push_back(new SurfFeatureDescriptor64(string(buff)));}
	matchers.clear();
	backing.clear();
	matchers.push_back(new BowAICKv2(200,5 ,0.3,scaling));	backing.push_back(steps);
	
	for(double j = 0.2; j <= 0.3; j+=0.005){
		sprintf(buff,"bowAICKsurf_wordthreshold_bl_%i",int(1000*j));
